/**
 * Search Plan — Phase 1 · Step 2
 *
 * Validates domain rules and builds an immutable SearchPlan from SearchInput.
 * Pure, deterministic, no DB/HTTP. Used by all downstream search stages.
 */
 ...
 *
 * Validations:
 * - arrival (checkIn) is before departure (checkOut)
 * - stay length ≥ 1 night
 * - total guest count > 0 (adults ≥ 0, children ≥ 0)
 *
 * Normalizations:
 * - dates → Date objects and ISO YYYY-MM-DD strings
 * - nights, guests, page, pageSize with defaults and clamping
 *
 * Derivation:
 * - geo: from input.latitude and input.longitude (required, validated)
 * - radius: from filters.radiusKm or filters.radius; default 5 km, clamped 1–500
 * - hasChildren: (children ?? 0) > 0
 *
 * @throws SearchPlanValidationError with a domain error code when validation fails
 */
